...
function createMockStorage(index, featureDetails, rfeDetails) {
  const store = {
    'releases/execution/index.json': index
  }
  if (featureDetails) {
...
describe('runPipeline', () => {
  ...
  it('accumulates warnings for empty index', () => {
    ...
    expect(result.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining('Feature index is empty')
      ])
    )
  })
  ...
  it('returns missingOutcomes array listing keys not found in index', () => {
    ...
    expect(result.warnings).not.toEqual(
      expect.arrayContaining([expect.stringContaining('not found in feature index')])
    )
  })
